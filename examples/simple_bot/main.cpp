...
#define FIRMWARE_VER_TEXT   "simple_bot v1"

// bit in ContactInfo.flags (bit 0 is reserved for BaseChatMesh's own 'favourite' use)
#define CONTACT_FLAG_GREETED   0x02
...
protected:
  ...
  static void normalizeCommand(const char* text, char* dest, size_t dest_len) {
    while (*text == ' ') text++;  // skip leading spaces
    size_t n = 0;
    while (*text && n < dest_len - 1) {
      dest[n++] = tolower((unsigned char) *text++);
    }
    while (n > 0 && dest[n - 1] == ' ') n--;  // trim trailing spaces
    dest[n] = 0;
  }

  void buildHelpText(char* dest, size_t dest_len) {
    snprintf(dest, dest_len,
      "Welcome to %s! Commands:\n"
      "ping - check the bot is alive\n"
      "time - current UTC time\n"
      "route - path info to you\n"
  ...
  void replyTo(const ContactInfo& contact, const char* text) {
    uint32_t expected_ack, est_timeout;
    int result = sendMessage(contact, getRTCClock()->getCurrentTime(), 0, text, expected_ack, est_timeout);
    if (result == MSG_SEND_FAILED) {
      Serial.println("   ERROR: unable to send reply.");
    }
  }
  ...
  void onMessageRecv(const ContactInfo& from, mesh::Packet* pkt, uint32_t sender_timestamp, const char *text) override {
    ...
    char cmd[32];
    normalizeCommand(text, cmd, sizeof(cmd));

    ContactInfo* contact = lookupContactByPubKey(from.id.pub_key, PUB_KEY_SIZE);
    bool first_contact = contact != NULL && (contact->flags & CONTACT_FLAG_GREETED) == 0;

    char reply[MAX_TEXT_LEN];

    if (first_contact || strcmp(cmd, "help") == 0) {
      ...
      snprintf(reply, sizeof(reply), "%04d-%02d-%02d %02d:%02d:%02d UTC",
        dt.year(), dt.month(), dt.day(), dt.hour(), dt.minute(), dt.second());
    } else if (strcmp(cmd, "route") == 0) {
      buildRouteText(from, pkt, reply, sizeof(reply));
    } else {
      snprintf(reply, sizeof(reply), "Unknown command. Send 'help' for the list.");
    }
